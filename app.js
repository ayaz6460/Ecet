...
function setupEventListeners() {
    ...
    // 23. Export Excel Button
    const rfExportExcelBtn = document.getElementById('rfExportExcelBtn');
    if (rfExportExcelBtn) {
        rfExportExcelBtn.addEventListener('click', () => {
            exportRankFinderToExcel();
        });
    }
}
...
// Export Rank Finder results to Excel (.xlsx via CSV with UTF-8 BOM — opens natively in Excel)
function exportRankFinderToExcel() {
    const rank = rankFinderCurrentRank;
    if (!rank) {
        alert('Please search for a rank first before exporting.');
        return;
    }

    // Retrieve and filter eligible colleges (same logic as PDF export)
    const stats = getActiveRFStats();
    let eligible = stats.filter(c => c.closing >= rank);
    ...
    if (branchFilter) eligible = eligible.filter(c => c.branch === branchFilter);
    if (typeFilter)   eligible = eligible.filter(c => (c.collegeType || 'Co-Education') === typeFilter);

    // Sort by rating (highest to lowest)
    eligible.sort((a, b) => (b.rating || 0) - (a.rating || 0));

    if (eligible.length === 0) {
        alert('No eligible colleges to export with the current filters.');
        return;
    }

    // Build CSV rows
    const typeText   = typeFilter   || 'All Colleges';
    const branchText = branchFilter || 'All Branches';

    // Header metadata rows
    const metaRows = [
        ['AllotIQ Portal - Eligible Colleges List'],
        [`Rank: ${rank.toLocaleString()}`, `Type: ${typeText}`, `Branch: ${branchText}`, `Total: ${eligible.length} colleges`],
        ['Generated By AllotIQ Portal | Based on 2025 ECET admission data | Developed By NotAyaz...'],
        [] // blank separator
    ];
    ...
    // Data rows
    const dataRows = eligible.map((c, idx) => {
        // Calculate admission chance label
        let chanceLabel = 'Low';
        const gap = c.closing - rank;
        if (gap >= 500)       chanceLabel = 'Very High';
        else if (gap >= 200)  chanceLabel = 'Good';
        else if (gap >= 0)    chanceLabel = 'Borderline';
        ...
        return [
            ...
            c.name,
            c.branch,
            c.collegeType || 'Co-Education',
            c.opening !== Infinity  ? c.opening  : 'N/A',
            c.closing !== -Infinity ? c.closing  : 'N/A',
            c.count,
            c.rating > 0 ? c.rating.toFixed(1) : 'N/A',
            chanceLabel,
            c.address || 'N/A'
    ...
    // Escape a cell value for CSV (quote if it contains comma, newline, or double-quote)
    ...
    const link = document.createElement('a');
    link.href     = url;
    link.download = `AllotIQ_Colleges_Rank_${rank}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
